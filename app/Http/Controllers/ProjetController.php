...
public function planifier(Request $request)
{
    ...
    // Vérifiez que l'utilisateur est authentifié et a les autorisations nécessaires
    $user = auth()->user();
    if (!$user || !$user->approbateur) {
        return redirect()->route('projets.index')->with('error', 'Vous devez être un approbateur pour accéder à cette page.');
    }

    // Récupérez les projets approuvés sans tâches associées
    $projects = EtudeProject::where('status', 'approved')
        ->whereDoesntHave('tasks') // Assurez-vous que cette relation est définie dans le modèle
        ->get();
...
// Sauvegarder la planification du projet
public function storePlanning(Request $request)
{
    $validated = $request->validate([
        'etude_project_id' => 'required|exists:etude_projects,id',
    ...
    foreach ($validated['tasks'] as $task) {
        Task::create([
            'etude_project_id' => $validated['etude_project_id'],
            'name' => $task['name'],
    ...
    return redirect()->route('projets.planifier')->with('success', 'Planification enregistrée avec succès.');
}
        ...
        public function showPlanning(Request $request)
        {
            ...
            // Récupérer le projet sélectionné et ses tâches
            $selectedProject = EtudeProject::findOrFail($projectId);
            $tasks = $selectedProject->tasks;

            return view('etudes_projets.plan', compact('selectedProject', 'tasks', 'projects'));
        }

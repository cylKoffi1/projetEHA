...
<script>
  (function() {
    ...
    function showAlertWhenReady(){
      ...
      } else { setTimeout(showAlertWhenReady, 50); }
    }
...
<style>
  /* ---------- TABLE ---------- */
  .tableClass { width: 100%; border-collapse: separate; }
  .tableClass th, .tableClass td { white-space: nowrap; overflow: hidden; text-overflow: ellipsis; min-width: 110px; }
  .tableClass tbody td:first-child,
  .tableClass thead tr:nth-child(1) th:first-child,
  .tableClass thead tr:nth-child(2) th:first-child { min-width: 260px; max-width: 340px; }
  .tableClass thead tr.group-row th { pointer-events: none; }
  tbody tr td a { text-decoration: none; }

  .national-row { position: sticky; top: 0; z-index: 2; background: #e8f4ff !important; font-weight: 600; }
  .national-row td, .national-row th { background: #e8f4ff !important; border-bottom: 2px solid #bcdfff; }
  ...
  /* Optionnel : tiret discret si cellule vide (reste aligné à droite) */
  .tableClass td:not(:first-child):empty::after {
    content: '-';
    opacity: .6;
  }

  /* lignes ratio par acteur */
        ...
        <div class="collapse" id="filtersCollapse">
          <div class="card-body">
            <div class="row g-4">
              ...
              {{-- TYPE PROJET --}}
              <div class="col-md-6 col-lg-4">
                <h6 class="small fw-bold text-muted mb-3 text-center"><i class="bi bi-grid-1x2 text-success me-1"></i> Type de projet</h6>
                <div class="row row-cols-3 g-2">
              ...
              <div class="col-md-12 col-lg-4">
                <h6 class="small fw-bold text-muted mb-2"><i class="bi bi-people text-warning me-1"></i> Acteurs</h6>
                <div class="row row-cols-3 g-2">
            ...
            <div class="table-responsive">
              <table class="table table-striped table-bordered tableClass" id="table1">
                ...
                <tbody>
                  ...
                  {{-- Gabarit invisible (repère de colonnes) pour créer les lignes ratio par JS --}}
...
<script>
  ...
  $(document).ready(async function () {
    ...
    // Donne un ID si absent
    let tableId = $table.attr('id');
    if (!tableId) { tableId = 'dt-' + Math.random().toString(36).slice(2, 8); $table.attr('id', tableId); }

    // Init DataTables (votre helper)
    await initDataTable(
      '{{ auth()->user()->acteur?->libelle_court }} {{ auth()->user()->acteur?->libelle_long }}',
      tableId,
      'Tableau de bord en nombre de projets'
    );
    const dt = $table.DataTable();

    /* ------------------ MAP COLONNES ------------------ */
    const statusOrder = @json($statusOrder);
    const colMap = {}; let start = 1; // 0 = libellé
    ...
    function getCellNumberByDom(rowEl, colIdx){
      ...
      return td ? parseIntFromText(td.textContent) : 0;
    }
    ...
    // Parse un entier depuis la cellule (vous avez déjà parseIntFromText/getCellNumberByDom)
    function actorRowHasAnyData(rowEl) {
      const colCount = dt.columns().count();
      for (let i = 1; i < colCount; i++) {               // ignore la 1ère colonne (libellé)
        if (!dt.column(i).visible()) continue;           // ne regarde que les colonnes visibles
    ...
    // Cache/affiche les lignes acteurs & leurs ratios si aucune donnée visible
    function hideEmptyActorRows() {
      ...
      $tbody.find('tr').each(function(){
        ...
        // ligne ratio suivante si présente
        ...
        if (hasData) {
          ...
          if (isRatioNext) $next.get(0).style.display = '';
        } else {
      ...
      // 2) Bonus : si une ligne ratio ne contient que des '-' (après calcul), on la cache
      $tbody.find('tr.ratio-actor-row').each(function(){
        ...
        const onlyDashesOrEmpty = tds.every(td => {
    ...
    /* ------------------ FILTRE ACTEURS (inclut les lignes ratio-*) ------------------ */
    ...
    function applyActorFilter(){ enabledActors = new Set($('.actor-filter:checked').map((_,el)=>el.value.toLowerCase()).get()); dt.draw(false); }
    ...
    /* ------------------ SYNCHRO COLS VISIBLES -> LIGNES RATIO ------------------ */
    function syncRatioColsToDtVisibility(){
      const colCount = dt.columns().count();
      for (let i=0; i<colCount; i++){
        const vis = dt.column(i).visible();
        const nth = i+1;
        $table.find(`tbody tr.ratio-actor-row td:nth-child(${nth})`)[vis?'show':'hide']();
      }
    }

    /* ------------------ ENTÊTES GROUPÉES (original + clones) ------------------ */
    function refreshGroupedHeader() {
      const $allTheads = $table.closest('.dataTables_wrapper').find('thead');
      $allTheads.each(function(){
        const $thead = $(this);
        const groupHeaderThs = $thead.find('tr.group-row th').toArray().slice(1);
        statusOrder.forEach((k, idx) => {
          ...
          if (!th) return;
          if (visibleCount === 0) { th.style.display='none'; th.setAttribute('colspan','0'); }
          else { th.style.display=''; th.setAttribute('colspan', String(visibleCount)); }
        });
      });
    }

    /* ------------------ CALCUL RATIOS (acteur / national) ------------------ */
    ...
    dt.on('draw.dt', function(){
      ...
      if ($nat.length) $nat.prependTo($body); // National en haut
      ensureActorRatioRows();                 // crée les lignes ratio manquantes
      recomputeActorRatios();                 // calcule les %
      refreshGroupedHeader();
      syncRatioColsToDtVisibility();
      hideEmptyActorRows();
    });

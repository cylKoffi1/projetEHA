...
@php
    ...
    $routeBase = ($typeVue ?? 'nombre') === 'finance' ? 'projet.finance.data' : 'projet.nombre.data';
...
<style>
    ...
    .kpi-card:hover { transform: translateY(-5px); box-shadow: 0 10px 20px rgba(0,0,0,.2); }
</style>
    ...
    <div class="page-heading">
        ...
        <div class="card shadow-sm mb-4">
            <div class="card-body">
                ...
                <form method="GET" action="{{ route('projet.dashboard') }}">
    ...
    {{-- ===== KPI CARDS ===== --}}
            ...
            <div class="card shadow-sm kpi-card" style="background: linear-gradient(to right, rgba(104,155,225,.9), #e7f1ff);">
                <div class="card-body py-3 d-flex justify-content-between align-items-center text-dark">
                    <div>
                        ...
                        <h6 class="mb-0">{{ number_format(($projectStatusCounts['Prévu'] ?? 0)
                            + ($projectStatusCounts['En cours'] ?? 0)
                            + ($projectStatusCounts['Clôturés'] ?? 0)
                            + ($projectStatusCounts['Terminé'] ?? 0)
                            + ($projectStatusCounts['Suspendu'] ?? 0)
                            + ($projectStatusCounts['Annulé'] ?? 0)
                            + ($projectStatusCounts['Redémarré'] ?? 0), 0, ',', ' ') }}</h6>
                    </div>
        ...
        @elseif(($mode ?? 'table') === 'graph')
            ...
            @php
                ...
                $tabOrder = ['INFRASTRUCTURE'];
                $tabs     = array_values(array_filter($tabOrder, fn($f) => $grouped->has($f)));
            @endphp

            {{-- ===== Graphiques globaux (mêmes que dash) ===== --}}
            <div class="row mt-3">
                <div class="col-md-6">
                    <div class="card shadow-sm">
                        <div class="card-body p-3">
                            <h6 class="text-center mb-2">Répartition des Projets par Statut</h6>
                            <canvas id="projectsStatusChart" style="max-height:220px;"></canvas>
                        </div>
                    </div>
                </div>
                <div class="col-md-6">
                    <div class="card shadow-sm">
                        <div class="card-body p-3">
                            <h6 class="text-center mb-2">Répartition en nombre d'Acteurs</h6>
                            <canvas id="actorsChart" style="max-height:220px;"></canvas>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="row mt-4">
                <div class="col-md-6">
                    <div class="card shadow-sm p-3">
                        <h6 class="text-center mb-2">Répartition par Financement (Global)</h6>
                        <canvas id="financementChart" style="max-height:220px;"></canvas>
                    </div>
                </div>
                <div class="col-md-6">
                    <div class="card shadow-sm p-3">
                        <h6 class="text-center mb-2">Nombre de projets par Année (Global)</h6>
                        <canvas id="anneeChart" style="max-height:220px;"></canvas>
                    </div>
                </div>
                </div>
            ...
            <div class="row mt-4">
                <div class="col-md-12">
                    <div class="card shadow-sm p-3">
                        <h6 class="text-center mb-2">Évolution du Budget Annuel en milliards (Global)</h6>
                        <canvas id="budgetChart" style="max-height:240px;"></canvas>
                    </div>
                </div>
            </div>
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function saveFilters() {
        ...
        localStorage.setItem("statFiltersProjet", JSON.stringify({
    ...
    function loadFilters() {
        const saved = JSON.parse(localStorage.getItem("statFiltersProjet") || "{}");
        if (saved.statuses) {
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        // Données globales (identiques au dash)
        ...
        const families        = @json($enabledFamilies ?? ['PROJET']);

        const elStatus = document.getElementById('projectsStatusChart');
        if (elStatus) createChart('projectsStatus', elStatus.getContext('2d'), {
            type: 'pie',
            data: { labels: Object.keys(statusData), datasets: [{ data: Object.values(statusData) }] },
            options: { responsive: true, plugins: { legend: { position: 'bottom' } } }
        });

        const elActors = document.getElementById('actorsChart');
        if (elActors) createChart('actorsChart', elActors.getContext('2d'), {
            type: 'bar',
            data: { labels: Object.keys(actorsData), datasets: [{ label: 'Nombre', data: Object.values(actorsData) }] },
            options: { responsive: true, plugins: { legend: { display: false } }, scales: { y: { beginAtZero: true } } }
        });

        const elFin = document.getElementById('financementChart');
        if (elFin) createChart('financementChart', elFin.getContext('2d'), {
            type: 'doughnut',
            data: { labels: (financementData||[]).map(f=>f.libelle), datasets: [{ data: (financementData||[]).map(f=>f.total_projets) }] },
            options: { responsive: true, plugins: { legend: { position: 'bottom' } } }
        });

        const elAnnee = document.getElementById('anneeChart');
        if (elAnnee) createChart('anneeChart', elAnnee.getContext('2d'), {
            type: 'line',
            data: { labels: Object.keys(anneeData), datasets: [{ label:'Projets', data:Object.values(anneeData), fill:false, tension:.3 }] },
            options: { responsive: true, scales: { x: { type: 'category' }, y: { beginAtZero: true } } }
        });

        const elBudget = document.getElementById('budgetChart');
        if (elBudget) createChart('budgetChart', elBudget.getContext('2d'), {
            type: 'bar',
            data: { labels: Object.keys(budgetData), datasets: [{ label:'Montant (Mds $)', data:Object.values(budgetData) }] },
            options: { responsive: true, plugins:{ legend:{ display:false } },
                scales: { x:{ type: 'category' }, y:{ beginAtZero:true, ticks:{ callback:v=> new Intl.NumberFormat().format(v)+' Md$' } } } }
        });

        // Ajustement des DataTables dans les onglets
        const userName = '{{ auth()->user()?->acteur?->libelle_court }} {{ auth()->user()?->acteur?->libelle_long }}';
        $('.dt-demandes').each(function () {
            const id = this.id;
            if (typeof initDataTable === 'function') {
                initDataTable(userName, id, 'Liste des projets');
            }
        });
...
{{-- Script d'alerte mot de passe --}}
    @if(session('force_password_change'))
        <script>
            Swal && Swal.fire({
                title: 'Bienvenue ! 👋',
                html: `<p>Pour sécuriser votre compte, merci de modifier votre mot de passe.</p>`,
                icon: 'info', confirmButtonText: 'OK', width: '600px'
            });
        </script>
    @endif

    ...
    <div class="card-body">
      <div class="row g-3">
        ...
        <div class="col-md-3">
          ...
          {{-- disabled: on lit la valeur en JS, et le contrôleur peut hydrater depuis la session si vide --}}
        ...
        <div class="col-md-3">
          ...
          <select id="wf-groupe" class="form-select">
  ...
  document.querySelectorAll('#steps .step-item').forEach(s => {
    ...
    s.querySelectorAll('.approver').forEach(a => {
      ...
      approbateurs.push({
        ...
        reference_approbateur: ref, // ATTENTION: c’est bien le code_acteur quand type=ACTEUR
...
document.getElementById('btn-publish').addEventListener('click', async function () {
  ...
  try {
    ...
    alert(data.message || 'Version publiée.');
  } catch (_) {
    alert('Erreur réseau ou serveur.');
  }
});
...
(function preload(){
  ...
  // pays (priorité: prefill -> contexte)
  ...
  // groupe (priorité: prefill -> contexte)
  ...
  // étapes (⚠️ utiliser mode_code, pas mode)
  (ver.etapes || []).forEach(e => addStep({
    position: e.position,
    mode_code: e.mode_code, // FIX
    quorum: e.quorum,
...
function enhanceRuleRow(row) {
  ...
  if ($fldSug && $fldSug.options.length <= 1) {
    for (const f of SUGGESTED_FIELDS) {
  ...
  row.querySelector('.btn-rbx-apply')?.addEventListener('click', () => {
    ...
    if (op === 'IN' || op === 'NOT_IN') {
      ...
      out = [parseSmart(raw1), parseSmart(raw2)];
    } else {
      out = parseSmart(raw1);
    }

    if (typeof out === 'string') $val.value = out;
    else $val.value = JSON.stringify(out);

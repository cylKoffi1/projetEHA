...
<script>
  ...
  async function uploadFiles(){
    ...
    try{
      ...
      const res = await fetch(`{{ route('projet.etude.temp.save.step5') }}`, { method:'POST', body: formData, headers:{'Accept':'application/json'} });
  ...
  function finaliserCodeProjet(){
    // Finalisation basée sur les données en session (les 5 étapes sauvegardées)
    fetch(`{{ isset($directMode) && $directMode ? route('projets.etude.finaliser.direct') : route('projets.finaliser') }}`, {

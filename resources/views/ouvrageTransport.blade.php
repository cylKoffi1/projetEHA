...
<section id="multiple-column-form">
                ...
                <div class="col-12 col-md-6 order-md-2 order-first">
                    <nav aria-label="breadcrumb" class="breadcrumb-header float-start float-lg-end">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active" aria-current="page">Ouvrage de transport</li>
            ...
            <div class="card">
                <div class="card-header">
                    <div style="display: flex; width: 100%; justify-content: space-between; align-items: center;">
                        <h5 class="card-title">
                            Ajout des ouvrages de transport
                ...
                <a  href="#" data-toggle="modal" data-target="#niveauAccesDonnees-modal" style="margin-left: 15px;"><i class="bi bi-plus-circle me-1"></i></a>
                    ...
                    <div style="text-align: center;">
                        <h5 class="card-title"> Liste des ouvrages de transport</h5>
                    </div>
                </div>
                <div class="card-content">
                    <div class="card-body">
                        <table class="table table-striped table-bordered" cellspacing="0" style="width: 100%" id="table1">
                            ...
                            <tbody>
                                @foreach ($ouvrageTransport as $ouvrageTransport)
                                <tr>
                                    <td>{{ $ouvrageTransport->code }}</td>
                                    <td>{{ $ouvrageTransport->libelle }}</td>
                                    <td>
                                        <div class="dropdown">
                                            ...
                                            <ul class="dropdown-menu z-3" aria-labelledby="userDropdown">
                                                <li><a class="dropdown-item" href="#" onclick="showEditOuvrageTransport('{{ $ouvrageTransport->code }}')"><i class="bi bi-pencil-square me-3"></i> Modifier</a></li>
                                                <li><a class="dropdown-item" href="#" onclick="deleteOuvrageTransport('{{ $ouvrageTransport->code }}')"> <i class="bi bi-trash3-fill me-3"></i> Supprimer</a></li>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="modalTitle">Enregistrement d'outils de transport'</h5>
                        ...
                        <div class="row match-height">
                            <div class="col-12">
                                <div class="card">
                                    <div class="card-content">
                                        <div class="card-body">
                                            <form class="form" method="POST" action="{{ route('ouvrageTransport.store') }}" data-parsley-validate>
                                                @csrf
    ...
    <div class="modal fade" id="niveauAccesDonnees-modal-edit" tabindex="-1" role="dialog" aria-labelledby="modalTitle" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="modalTitle">Modification d'outils de transport'</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                        ...
                        <div class="row match-height">
                            <div class="col-12">
                                <div class="card">
                                    <div class="card-content">
                                        <div class="card-body">
                                            <form class="form" method="POST" action="{{ route('ouvrageTransport.update') }}" data-parsley-validate>
                                                @csrf
                        <input type="hidden" class="form-control" id="ecran_id" value="{{ $ecran->id }}"  name="ecran_id" required>
                                                <div class="row">
                                                    <div class="col-md-6 col-12">
...
<script>
    ...
    $(document).ready(function() {
        initDataTable('{{ auth()->user()->personnel->nom }} {{ auth()->user()->personnel->prenom }}', 'table1', 'Liste des ouvrages de transport');
        ...
        $('#code').on('input', function() {
            ...
            $.ajax({
                url: '/check-ouvrageTransport-code', // Replace with the actual URL in your Laravel routes
                method: 'POST'
                , data: {
    ...
    function showEditOuvrageTransport(code) {
        $('#niveauAccesDonnees-modal-edit').modal('show');
        $.ajax({
            type: 'GET'
            , url: '/admin/ouvrageTransport/' + code
    ...
    function deleteOuvrageTransport(code) {
        if (confirm("Êtes-vous sûr de vouloir supprimer cet ouvrage de Transport ?")) {
            $.ajax({
                url: '/admin/ouvrageTransport/delete/' + code
                , method: 'DELETE', // Utilisez la méthode DELETE pour la suppression
                data: {
                    _token: '{{ csrf_token() }}' // Assurez-vous d'envoyer le jeton CSRF
                }
                , success: function(response) {
                    var message = "Ouvrage de Transport supprimé avec succès.";
                    showPopup(message);

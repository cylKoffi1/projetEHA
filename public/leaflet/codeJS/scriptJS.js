...
var statesDataRegionsGeoJs = L.geoJson(statesDataRegions, {
    ...
    onEachFeature: function (feature, layer) {
        layer.on({
            ...
            // Ajoutez cet écouteur d'événements lorsque vous initialisez la carte
            map.on('contextmenu', function (e) {
                e.originalEvent.preventDefault(); // Empêche le comportement par défaut (faire glisser la carte)

                // Appelez votre fonction pour créer et afficher le menu contextuel
                var geoCode = getGeoCodeFromLatLng(e.latlng); // Obtenez geoCode à partir des coordonnées latlng
                var geoType = getGeoTypeFromLatLng(e.latlng); // Obtenez geoType à partir des coordonnées latlng
                var props = getPropsFromLatLng(e.latlng); // Obtenez props à partir des coordonnées latlng

                var contextMenu = createContextMenu(contextMenuItems, geoCode, geoType, props);
